...
下表提供了有关用于在客户门户网站中编辑用户概要文件的其他设置的信息。有关每种帐户的更多详细信息，请参阅后面的各部分。

|字段|定义|
|-----|----------|
|名字，姓氏|与用户概要文件相关联的用户的名字和姓氏。|
|电子邮件地址|用于接收 {{site.data.keyword.BluSoftlayer_notm}} 基础架构中有关该帐户的通知的首选电子邮件地址。更改电子邮件地址会更改 {{site.data.keyword.BluSoftlayer_notm}} 基础架构中的记录。此更改不会影响与 IBM 标识认证凭证的链接。您必须在 IBM 标识概要文件中更改 IBM 标识的电子邮件地址。|
...
提交更改后，会立即更新用户许可权。如果授予了许可权，那么用户可以查看所选功能并与其进行交互。如果除去了许可权，那么用户不能查看所选功能，也不能与其进行交互。通过重复上述步骤，可以随时再次更新许可权。
...
<dd>帐户上的管理员已取消此用户，取消操作正在处理中。这是系统生成的状态。</dd>
</dl>
